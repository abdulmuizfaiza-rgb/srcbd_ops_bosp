                ...
                @else
                                        ...
                                        <tr x-show="terbuka" x-cloak>
                                            <td colspan="3" class="px-4 py-3 bg-white border border-blue-100">
                                                <div class="max-w-2xl space-y-3">
                                                    @foreach ($labelTab2Tunggal as $field => $label)
                                                    ...
                                                    {{-- Grup Tarik Tunai TW N + Saldo TW N (Saldo Kas Bank + Saldo
                                                         Kas Tunai) sejajar di sebelah KANAN Tarik Tunai TW N yang
                                                         bersangkutan, sesuai permintaan user 2026-09-16 (Part 31). --}}
                                                    @for ($tw = 1; $tw <= 4; $tw++)
                                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
                                                            <div class="flex items-center gap-3">
                                                                <span class="text-xs font-medium text-slate-600 sm:w-32">Tarik Tunai TW {{ $tw }}</span>
                                                                <x-honor-ptk-tarif-cell :row-id="$sekolah->id" :field="'tarik_tunai_tw'.$tw" :value="$baris[$sekolah->id]['tarik_tunai_tw'.$tw] ?? ''" :revisi="$revisi" class="w-28" />
                                                            </div>
                                                            <div class="flex items-end gap-3 sm:pl-4 sm:border-l sm:border-slate-200">
                                                                <div class="flex flex-col items-end gap-1">
                                                                    <span class="text-[10px] text-slate-500">Saldo Kas Bank</span>
                                                                    <x-honor-ptk-tarif-cell :row-id="$sekolah->id" :field="'saldo_kas_bank_tw'.$tw" :value="$baris[$sekolah->id]['saldo_kas_bank_tw'.$tw] ?? ''" :revisi="$revisi" class="w-24" />
                                                                </div>
                                                                <div class="flex flex-col items-end gap-1">
                                                                    <span class="text-[10px] text-slate-500">Saldo Kas Tunai</span>
                                                                    <x-honor-ptk-tarif-cell :row-id="$sekolah->id" :field="'saldo_kas_tunai_tw'.$tw" :value="$baris[$sekolah->id]['saldo_kas_tunai_tw'.$tw] ?? ''" :revisi="$revisi" class="w-24" />
                                                                </div>
                                                                <div class="flex flex-col items-end gap-1">
                                                                    <span class="text-[10px] font-semibold text-slate-600">Saldo TW {{ $tw }}</span>
                                                                    <span class="text-xs font-bold text-slate-800 bg-yellow-50 border border-yellow-200 rounded px-2 py-1.5 w-24 text-right">
                                                                        Rp {{ number_format((int) ($baris[$sekolah->id]['saldo_tw'.$tw] ?? 0), 0, ',', '.') }}
                                                                    </span>
                                                                </div>
                                                            </div>
                                                        </div>
                            ...
                            <div class="mt-4 border-t-2 border-blue-200 pt-4">
                                ...
                                <div class="max-w-2xl space-y-3">
                                    ...
                                    @for ($tw = 1; $tw <= 4; $tw++)
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
                                            <div class="flex items-center gap-3">
                                                <span class="text-xs font-semibold text-slate-700 sm:w-32">Tarik Tunai TW {{ $tw }}</span>
                                                <span class="text-xs font-bold text-slate-800 w-28 text-right">Rp {{ number_format($totalTab2['tarik_tunai_tw'.$tw] ?? 0, 0, ',', '.') }}</span>
                                            </div>
                                            <div class="flex items-end gap-3 sm:pl-4 sm:border-l sm:border-slate-200">
                                                <div class="flex flex-col items-end gap-1">
                                                    <span class="text-[10px] text-slate-500">Saldo Kas Bank</span>
                                                    <span class="text-xs font-bold text-slate-800 w-24 text-right">Rp {{ number_format($totalTab2['saldo_kas_bank_tw'.$tw] ?? 0, 0, ',', '.') }}</span>
                                                </div>
                                                <div class="flex flex-col items-end gap-1">
                                                    <span class="text-[10px] text-slate-500">Saldo Kas Tunai</span>
                                                    <span class="text-xs font-bold text-slate-800 w-24 text-right">Rp {{ number_format($totalTab2['saldo_kas_tunai_tw'.$tw] ?? 0, 0, ',', '.') }}</span>
                                                </div>
                                                <div class="flex flex-col items-end gap-1">
                                                    <span class="text-[10px] font-semibold text-slate-600">Saldo TW {{ $tw }}</span>
                                                    <span class="text-xs font-bold text-slate-800 bg-yellow-100 border border-yellow-200 rounded px-2 py-1.5 w-24 text-right">Rp {{ number_format($totalTab2['saldo_tw'.$tw] ?? 0, 0, ',', '.') }}</span>
                                                </div>
                                            </div>
                                        </div>
                                    @endfor
                    ...
                    @else
                        {{-- Admin BOSP: form vertikal untuk sekolahnya sendiri saja,
                             ...
                             jawaban AskUserQuestion 2026-09-16. Grup Tarik Tunai TW N +
                             Saldo TW N memakai layout yang sama seperti blok expand
                             Superadmin di atas (Part 31). --}}
                                ...
                                <div class="space-y-3">
                                    ...
                                    @for ($tw = 1; $tw <= 4; $tw++)
                                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-slate-100 pb-3 last:border-b-0 last:pb-0">
                                            <div class="flex items-center gap-3">
                                                <span class="text-xs font-medium text-slate-600 sm:w-32">Tarik Tunai TW {{ $tw }}</span>
                                                <x-honor-ptk-tarif-cell :row-id="$sekolahSaya->id" :field="'tarik_tunai_tw'.$tw" :value="$baris[$sekolahSaya->id]['tarik_tunai_tw'.$tw] ?? ''" :revisi="$revisi" class="w-28" />
                                            </div>
                                            <div class="flex items-end gap-3 sm:pl-4 sm:border-l sm:border-slate-200">
                                                <div class="flex flex-col items-end gap-1">
                                                    <span class="text-[10px] text-slate-500">Saldo Kas Bank</span>
                                                    <x-honor-ptk-tarif-cell :row-id="$sekolahSaya->id" :field="'saldo_kas_bank_tw'.$tw" :value="$baris[$sekolahSaya->id]['saldo_kas_bank_tw'.$tw] ?? ''" :revisi="$revisi" class="w-24" />
                                                </div>
                                                <div class="flex flex-col items-end gap-1">
                                                    <span class="text-[10px] text-slate-500">Saldo Kas Tunai</span>
                                                    <x-honor-ptk-tarif-cell :row-id="$sekolahSaya->id" :field="'saldo_kas_tunai_tw'.$tw" :value="$baris[$sekolahSaya->id]['saldo_kas_tunai_tw'.$tw] ?? ''" :revisi="$revisi" class="w-24" />
                                                </div>
                                                <div class="flex flex-col items-end gap-1">
                                                    <span class="text-[10px] font-semibold text-slate-600">Saldo TW {{ $tw }}</span>
                                                    <span class="text-xs font-bold text-slate-800 bg-yellow-50 border border-yellow-200 rounded px-2 py-1.5 w-24 text-right">
                                                        Rp {{ number_format((int) ($baris[$sekolahSaya->id]['saldo_tw'.$tw] ?? 0), 0, ',', '.') }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
