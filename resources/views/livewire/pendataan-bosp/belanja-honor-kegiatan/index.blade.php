...
<div>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                {{-- Tab UTAMA (Honor Kegiatan / Belanja Makan & Minum / Belanja
                     Perjalanan Dinas) - lapis tab PALING ATAS, baru di
                     bawahnya ada tab Triwulan (permintaan user 2026-09-11,
                     Part 20). Triwulan aktif TIDAK direset saat pindah tab
                     utama (lihat Index::pindahTabUtama()). --}}
                ...
                {{-- Tab Triwulan - user tidak menentukan warna khusus untuk menu
                     ...
                     sesuai permintaan user (2026-09-11 & lanjutan Part 20),
                     berubah mengikuti tab utama yang aktif. --}}
                    ...
                    <nav class="flex flex-wrap gap-3">
                        @foreach ($triwulanOptions as $value => $label)
                            @php
                                ...
                                $labelTriwulan = match ($tabUtama) {
                                    \App\Models\BelanjaHonorKegiatan::JENIS_MAKAN_MINUM => 'Belanja Makan & Minum',
                                    \App\Models\BelanjaHonorKegiatan::JENIS_PERJALANAN_DINAS => 'Belanja Perjalanan Dinas',
                    ...
                    <div class="overflow-auto scrollbar-modern border border-slate-200 rounded-lg{{ $terkunciTriwulanIni ? ' pointer-events-none opacity-60 select-none' : '' }}" style="max-height: 32rem; zoom: {{ $zoomPercent }}%;">
                        <table class="min-w-full divide-y divide-slate-200 text-xs">
                            ...
                            @forelse ($daftarSekolah as $sekolah)
                                @php
                                    $nomorSekolah++;
                                    $jumlahBaris = $sekolah->belanjaHonorKegiatan->count();
                                    $nomorBaris = 0;
                                @endphp
                                <tbody wire:key="honor-kegiatan-grup-{{ $sekolah->id }}" x-data="{ terbuka: false }" class="divide-y divide-slate-100 border-b-2 border-slate-200">
                                    ...
                                    @foreach ($sekolah->belanjaHonorKegiatan as $row)
                                        @php $nomorBaris++; $revisi = $revisiBaris[$row->id] ?? 0; @endphp
                                        <tr wire:key="honor-kegiatan-baris-{{ $row->id }}" x-show="terbuka" x-cloak>
                                            <td class="px-2 py-2">
                                            ...
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="honor-kegiatan-input-{{ $row->id }}-uraian" wire:model.blur="baris.{{ $row->id }}.uraian" class="w-40 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.uraian') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.uraian')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" inputmode="numeric" wire:key="honor-kegiatan-input-{{ $row->id }}-volume" wire:model.blur="baris.{{ $row->id }}.volume" class="w-16 text-xs text-right rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.volume') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.volume')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="honor-kegiatan-input-{{ $row->id }}-satuan" wire:model.blur="baris.{{ $row->id }}.satuan" class="w-20 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.satuan') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                    ...
                                    <tr wire:key="honor-kegiatan-baru-{{ $sekolah->id }}" class="bg-blue-50/40" x-show="terbuka" x-cloak>
                                        <td class="px-2 py-2"></td>
                                        <td class="px-2 py-2 whitespace-nowrap text-slate-300">&nbsp;</td>
                                        <td class="px-2 py-2 whitespace-nowrap text-slate-600">{{ $sekolah->npsn ?? '-' }}</td>
                                        <td class="px-2 py-2 whitespace-nowrap font-medium text-slate-800">{{ $sekolah->nama_sekolah ?? '-' }}</td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="honor-kegiatan-input-baru-{{ $sekolah->id }}-uraian" wire:model.blur="baris.{{ $idBaru }}.uraian" placeholder="Uraian" class="w-40 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.uraian') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.uraian')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" inputmode="numeric" wire:key="honor-kegiatan-input-baru-{{ $sekolah->id }}-volume" wire:model.blur="baris.{{ $idBaru }}.volume" placeholder="0" class="w-16 text-xs text-right rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.volume') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.volume')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="honor-kegiatan-input-baru-{{ $sekolah->id }}-satuan" wire:model.blur="baris.{{ $idBaru }}.satuan" placeholder="Satuan" class="w-20 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.satuan') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                    ...
                                    <tr wire:key="honor-kegiatan-total-sekolah-{{ $sekolah->id }}" class="bg-slate-100 font-bold text-slate-700 border-t-2 border-slate-300" x-show="terbuka" x-cloak>
                                        <td colspan="8" class="px-2 py-2 text-right">
                                            {{ match ($tabUtama) {
                                                \App\Models\BelanjaHonorKegiatan::JENIS_MAKAN_MINUM => 'Jumlah Belanja Makan & Minum',
                                                \App\Models\BelanjaHonorKegiatan::JENIS_PERJALANAN_DINAS => 'Jumlah Belanja Perjalanan Dinas',
                                                default => 'Jumlah Honor Kegiatan',
                                            } }}
                                        </td>
                                        <td class="px-2 py-2 whitespace-nowrap text-right">Rp {{ number_format((int) $sekolah->belanjaHonorKegiatan->sum('jumlah'), 0, ',', '.') }}</td>
                                        <td colspan="2"></td>
                                    </tr>
                                </tbody>
                            @empty
                            ...
                            @if ($daftarSekolah->isNotEmpty())
                                <tbody>
                                    <tr class="bg-slate-200 font-bold text-slate-800 border-t-2 border-slate-400">
                                        <td colspan="8" class="px-2 py-2.5 text-right">
                                            {{ match ($tabUtama) {
                                                \App\Models\BelanjaHonorKegiatan::JENIS_MAKAN_MINUM => 'Jumlah Belanja Makan & Minum Seluruh Sekolah',
                                                \App\Models\BelanjaHonorKegiatan::JENIS_PERJALANAN_DINAS => 'Jumlah Belanja Perjalanan Dinas Seluruh Sekolah',
                                                default => 'Jumlah Honor Kegiatan Seluruh Sekolah',
                                            } }}
                                        </td>
                                        <td class="px-2 py-2.5 whitespace-nowrap text-right">Rp {{ number_format((int) $totalJumlahKeseluruhan, 0, ',', '.') }}</td>
                                        <td colspan="2"></td>
                                    </tr>
                                </tbody>
                            @endif
    ...
    <x-modal name="belanja-honor-kegiatan-form" :show="$showForm" maxWidth="lg">
        @php
            $labelJenisModal = match ($tabUtama) {
                \App\Models\BelanjaHonorKegiatan::JENIS_MAKAN_MINUM => 'Belanja Makan & Minum',
                \App\Models\BelanjaHonorKegiatan::JENIS_PERJALANAN_DINAS => 'Belanja Perjalanan Dinas',
                default => 'Belanja Honor Kegiatan',
            };
        ...
        <form wire:submit="simpan" class="p-6">
            <h2 class="text-lg font-medium text-slate-900 mb-1">
                {{ $editingId ? 'Edit Data '.$labelJenisModal : 'Tambah Data '.$labelJenisModal }}
            </h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <x-text-input wire:model="uraian" id="uraian" class="block mt-1 w-full" type="text" placeholder="mis. Honor Panitia Kegiatan, Honor Narasumber" />
    ...
    <x-modal name="belanja-honor-kegiatan-hapus" :show="$confirmingDeleteId !== null" maxWidth="md">
        <div class="p-6">
    ...
    <x-modal name="belanja-honor-kegiatan-hapus-terpilih" :show="$confirmingHapusTerpilih" maxWidth="md">
        <div class="p-6">
