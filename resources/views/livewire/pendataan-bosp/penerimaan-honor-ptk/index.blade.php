                            ...
                            @forelse ($daftarSekolah as $sekolah)
                                ...
                                <tbody wire:key="honor-ptk-grup-{{ $sekolah->id }}" x-data="{ terbuka: false }" class="divide-y divide-slate-100 border-b-2 border-slate-200">
                                    ...
                                    @foreach ($sekolah->penerimaanHonorPtk as $row)
                                        ...
                                        <tr wire:key="honor-ptk-baris-{{ $row->id }}" x-show="terbuka" x-cloak>
                                            ...
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" inputmode="numeric" maxlength="16" wire:model.blur="baris.{{ $row->id }}.nuptk" class="w-32 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.nuptk') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.nuptk')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:model.blur="baris.{{ $row->id }}.nama_penerima" class="w-40 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.nama_penerima') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                    ...
                                    <tr wire:key="honor-ptk-baru-{{ $sekolah->id }}" class="bg-blue-50/40" x-show="terbuka" x-cloak>
                                        ...
                                        <td class="px-2 py-2 whitespace-nowrap font-medium text-slate-800">{{ $sekolah->nama_sekolah ?? '-' }}</td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" inputmode="numeric" maxlength="16" wire:model.blur="baris.{{ $idBaru }}.nuptk" placeholder="NUPTK" class="w-32 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.nuptk') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.nuptk')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:model.blur="baris.{{ $idBaru }}.nama_penerima" placeholder="Nama Penerima" class="w-40 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.nama_penerima') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
