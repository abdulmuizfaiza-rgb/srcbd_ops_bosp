...
<div>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                {{-- Tab UTAMA (Rincian Belanja Modal Peralatan & Mesin (KIB B) /
                     Rincian Belanja Modal Aset Tetap Lainnya (KIB E)) - lapis tab
                     PALING ATAS, baru di bawahnya ada tab Triwulan (permintaan user
                     2026-09-11, Part 21 poin 2). Triwulan aktif TIDAK direset saat
                     pindah tab utama (lihat Index::pindahTabUtama()). --}}
                ...
                {{-- Tab Triwulan - skema warna BAWAAN (Sky/Emerald/Amber/Fuchsia)
                     sama seperti menu lain, label teks berubah mengikuti tab utama
                     yang aktif. --}}
                    ...
                    <nav class="flex flex-wrap gap-3">
                        @foreach ($triwulanOptions as $value => $label)
                            @php
                                ...
                                $labelTab = match (true) {
                                    $tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD => 'BMD',
                                    $tabUtama === \App\Models\RincianBelanjaModal::JENIS_ASET_TETAP_LAINNYA => 'Belanja Modal Aset Tetap Lainnya (KIB E)',
                                    default => 'Belanja Modal Peralatan & Mesin (KIB B)',
                                };
                            @endphp
                            <button
                                type="button"
                                ...
                                @class([
                                    ...
                                    $skema['aktif'].' text-white shadow-lg ring-black/10 border-b-4 border-black/20 scale-105' => $triwulan === $value && ! $tampilRekapBmd,
                                    $skema['nonaktif'].' shadow-sm hover:shadow-md hover:scale-[1.02] border-b-2 border-black/5' => $triwulan !== $value || $tampilRekapBmd,
                                ])
                            >
                                {{ $labelTab }} TW-{{ $value }}
                            </button>
                        ...
                        {{-- Sub-tab KELIMA "Rekap BMD Tahun Anggaran {tahun}" (permintaan
                             user 2026-09-22) - HANYA muncul saat tab utama BMD aktif,
                             menggabungkan data TW-1 s.d. TW-4 tahun berjalan, read-only
                             (jawaban AskUserQuestion "Hanya tampil & export"). --}}
                        @if ($tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD)
                            <button
                                type="button"
                                wire:click="pindahKeRekapBmd"
                                @class([
                                    'px-4 py-2 rounded-xl text-sm font-bold tracking-wide transition-all duration-150 ring-1 bg-gradient-to-b',
                                    'from-indigo-400 to-indigo-600 text-white shadow-lg ring-black/10 border-b-4 border-black/20 scale-105' => $tampilRekapBmd,
                                    'from-indigo-50 to-indigo-100 text-indigo-700 ring-indigo-200 shadow-sm hover:shadow-md hover:scale-[1.02] border-b-2 border-black/5' => ! $tampilRekapBmd,
                                ])
                            >
                                Rekap BMD Tahun Anggaran {{ $tahun }}
                            </button>
                        @endif
                    </nav>
                </div>
                ...
                <div class="p-4 sm:p-8">
                    <div class="flex flex-col xl:flex-row xl:flex-wrap xl:items-start xl:justify-between gap-3 mb-6">
                        <div class="flex flex-wrap sm:flex-row sm:items-center gap-2.5">
                            ...
                            <input wire:model.live.debounce.300ms="search" type="text" placeholder="{{ $tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD ? 'Cari nama barang / sekolah...' : 'Cari nama barang / kode UPB / sekolah...' }}" class="border-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm text-xs">
                        ...
                        <div class="flex flex-wrap sm:flex-row sm:items-center gap-2.5">
                            {{-- Import & Tambah TIDAK ditampilkan pada sub-tab "Rekap BMD
                                 Tahun Anggaran" - read-only, HANYA lihat & Export (jawaban
                                 AskUserQuestion 2026-09-22). --}}
                            @unless ($tampilRekapBmd)
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                @unless ($tampilRekapBmd)
                                <x-primary-button type="button" wire:click="{{ $tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD ? 'tambahBmd' : 'tambah' }}" :disabled="$terkunciTriwulanIni" class="whitespace-nowrap !px-2.5 !py-1.5 !text-[10px]">
                                    <x-icon name="plus" class="w-3.5 h-3.5 mr-1" />
                                    Tambah
                                </x-primary-button>
                                {{-- Hapus Terpilih (hapus massal) - permintaan user 2026-09-26.
                                     Satu tombol dipakai KEDUA tab (jenis/BMD), pola ternary sama
                                     seperti tombol Tambah di atas - tab "jenis" pakai
                                     $dipilih/konfirmasiHapusTerpilih, tab "BMD" pakai
                                     $dipilihBmd/konfirmasiHapusTerpilihBmd (lihat Index.php). --}}
                                <x-danger-button type="button" wire:click="{{ $tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD ? 'konfirmasiHapusTerpilihBmd' : 'konfirmasiHapusTerpilih' }}" wire:loading.attr="disabled" :disabled="($tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD ? count($dipilihBmd) : count($dipilih)) === 0 || $terkunciTriwulanIni" class="whitespace-nowrap !px-2.5 !py-1.5 !text-[10px]">
                                    <x-icon name="trash" class="w-3.5 h-3.5 mr-1" />
                                    Hapus Terpilih ({{ $tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD ? count($dipilihBmd) : count($dipilih) }})
                                </x-danger-button>
                                @endunless
                            </div>
                        </div>
                    ...
                    @if ($tabUtama === \App\Models\RincianBelanjaModal::TAB_BMD)
                        @if ($tampilRekapBmd)
                            @include('livewire.pendataan-bosp.rincian-belanja-modal._tabel-rekap-bmd')
                        @else
                            @include('livewire.pendataan-bosp.rincian-belanja-modal._tabel-bmd')
                        @endif
                    @else
                    <div class="overflow-auto scrollbar-modern border border-slate-200 rounded-lg{{ $terkunciTriwulanIni ? ' pointer-events-none opacity-60 select-none' : '' }}" style="max-height: 32rem; zoom: {{ $zoomPercent }}%;">
                        <table class="min-w-full divide-y divide-slate-200 text-xs">
                            ...
                            {{-- Pola tbody-per-sekolah dengan simbol +/- di Kolom No, SAMA
                                 PERSIS seperti Rincian Pemeliharaan - diterapkan sejak awal
                                 di menu baru ini karena menu ini juga "banyak baris per
                                 sekolah" (jawaban AskUserQuestion 2026-09-11). --}}
                            @php $nomorSekolah = 0; @endphp
                            @forelse ($daftarSekolah as $sekolah)
                                @php
                                    $nomorSekolah++;
                                    $jumlahBaris = $sekolah->rincianBelanjaModal->count();
                                    $nomorBaris = 0;
                                @endphp
                                <tbody wire:key="rincian-belanja-modal-grup-{{ $sekolah->id }}" x-data="{ terbuka: false }" class="divide-y divide-slate-100 border-b-2 border-slate-200">
                                    ...
                                    @foreach ($sekolah->rincianBelanjaModal as $row)
                                        @php $nomorBaris++; $revisi = $revisiBaris[$row->id] ?? 0; @endphp
                                        <tr wire:key="rincian-belanja-modal-baris-{{ $row->id }}" x-show="terbuka" x-cloak>
                                            <td class="px-2 py-2">
                                            ...
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="rincian-belanja-modal-input-{{ $row->id }}-kode_upb" wire:model.blur="baris.{{ $row->id }}.kode_upb" class="w-24 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.kode_upb') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.kode_upb')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="rincian-belanja-modal-input-{{ $row->id }}-nama_barang" wire:model.blur="baris.{{ $row->id }}.nama_barang" class="w-36 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.nama_barang') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.nama_barang')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="rincian-belanja-modal-input-{{ $row->id }}-nama_merk_barang" wire:model.blur="baris.{{ $row->id }}.nama_merk_barang" class="w-28 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.nama_merk_barang') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.nama_merk_barang')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" inputmode="numeric" wire:key="rincian-belanja-modal-input-{{ $row->id }}-volume" wire:model.blur="baris.{{ $row->id }}.volume" class="w-16 text-xs text-right rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.volume') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.volume')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="rincian-belanja-modal-input-{{ $row->id }}-satuan" wire:model.blur="baris.{{ $row->id }}.satuan" class="w-20 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.satuan') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            ...
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="rincian-belanja-modal-input-{{ $row->id }}-asal_usul" wire:model.blur="baris.{{ $row->id }}.asal_usul" class="w-28 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.asal_usul') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.asal_usul')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="date" wire:key="rincian-belanja-modal-input-{{ $row->id }}-tanggal" wire:model.blur="baris.{{ $row->id }}.tanggal" class="text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.tanggal') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                                @error('baris.'.$row->id.'.tanggal')
                                                    <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                                @enderror
                                            </td>
                                            <td class="px-1 py-1.5 whitespace-nowrap">
                                                <input type="text" wire:key="rincian-belanja-modal-input-{{ $row->id }}-keterangan" wire:model.blur="baris.{{ $row->id }}.keterangan" class="w-28 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$row->id.'.keterangan') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                    ...
                                    {{-- Baris placeholder kosong siap-isi, SELALU ada di akhir tiap
                                         sekolah - pola sama seperti Rincian Pemeliharaan. Kunci
                                         NEGATIF (-ID sekolah); begitu salah satu kotaknya diisi &
                                    ...
                                    <tr wire:key="rincian-belanja-modal-baru-{{ $sekolah->id }}" class="bg-blue-50/40" x-show="terbuka" x-cloak>
                                        <td class="px-2 py-2"></td>
                                        <td class="px-2 py-2 whitespace-nowrap text-slate-300">&nbsp;</td>
                                        <td class="px-2 py-2 whitespace-nowrap text-slate-600">{{ $sekolah->npsn ?? '-' }}</td>
                                        <td class="px-2 py-2 whitespace-nowrap font-medium text-slate-800">{{ $sekolah->nama_sekolah ?? '-' }}</td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-kode_upb" wire:model.blur="baris.{{ $idBaru }}.kode_upb" placeholder="Kode UPB" class="w-24 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.kode_upb') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.kode_upb')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-nama_barang" wire:model.blur="baris.{{ $idBaru }}.nama_barang" placeholder="Nama Barang" class="w-36 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.nama_barang') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.nama_barang')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-nama_merk_barang" wire:model.blur="baris.{{ $idBaru }}.nama_merk_barang" placeholder="Merk" class="w-28 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.nama_merk_barang') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.nama_merk_barang')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" inputmode="numeric" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-volume" wire:model.blur="baris.{{ $idBaru }}.volume" placeholder="0" class="w-16 text-xs text-right rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.volume') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.volume')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-satuan" wire:model.blur="baris.{{ $idBaru }}.satuan" placeholder="Satuan" class="w-20 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.satuan') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                        ...
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-asal_usul" wire:model.blur="baris.{{ $idBaru }}.asal_usul" placeholder="Asal Usul" class="w-28 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.asal_usul') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.asal_usul')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="date" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-tanggal" wire:model.blur="baris.{{ $idBaru }}.tanggal" class="text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.tanggal') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                            @error('baris.'.$idBaru.'.tanggal')
                                                <span class="block text-[10px] text-red-500 mt-0.5 whitespace-normal">{{ $message }}</span>
                                            @enderror
                                        </td>
                                        <td class="px-1 py-1.5 whitespace-nowrap">
                                            <input type="text" wire:key="rincian-belanja-modal-input-baru-{{ $sekolah->id }}-keterangan" wire:model.blur="baris.{{ $idBaru }}.keterangan" placeholder="Keterangan" class="w-28 text-xs rounded px-1.5 py-1.5 border {{ $errors->has('baris.'.$idBaru.'.keterangan') ? 'border-red-400 focus:border-red-500 focus:ring-red-400' : 'border-dashed border-blue-300 focus:border-blue-500 focus:ring-blue-400' }} focus:ring-1 focus:outline-none">
                                    ...
                                    {{-- Baris Jumlah PER SEKOLAH (permintaan user 2026-09-16, Part
                                         ...
                                         seperti baris detail lain (x-show="terbuka"). Label
                                         mengikuti tab utama yang sedang aktif. --}}
                                    <tr wire:key="rincian-belanja-modal-total-sekolah-{{ $sekolah->id }}" class="bg-slate-100 font-bold text-slate-700 border-t-2 border-slate-300" x-show="terbuka" x-cloak>
                                        <td colspan="10" class="px-2 py-2 text-right">
                                            {{ $tabUtama === \App\Models\RincianBelanjaModal::JENIS_ASET_TETAP_LAINNYA ? 'Jumlah Rincian Belanja Modal Aset Tetap Lainnya (KIB E)' : 'Jumlah Belanja Modal Peralatan & Mesin (KIB B)' }}
                                        </td>
                                        <td class="px-2 py-2 whitespace-nowrap text-right">Rp {{ number_format((int) $sekolah->rincianBelanjaModal->sum('total_harga'), 0, ',', '.') }}</td>
                                        <td colspan="4"></td>
                                    </tr>
                                </tbody>
                            @empty
                            ...
                            {{-- Baris Jumlah SELURUH SEKOLAH - SELALU tampil (tidak ikut
                                 x-show sekolah manapun), label mengikuti tab utama aktif. --}}
                            @if ($daftarSekolah->isNotEmpty())
                                <tbody>
                                    <tr class="bg-slate-200 font-bold text-slate-800 border-t-2 border-slate-400">
                                        <td colspan="10" class="px-2 py-2.5 text-right">
                                            {{ $tabUtama === \App\Models\RincianBelanjaModal::JENIS_ASET_TETAP_LAINNYA ? 'Jumlah Rincian Belanja Modal Aset Tetap Lainnya (KIB E) Seluruh Sekolah' : 'Jumlah Belanja Modal Peralatan & Mesin (KIB B) Seluruh Sekolah' }}
                                        </td>
    ...
    <x-modal name="rincian-belanja-modal-form" :show="$showForm" maxWidth="lg">
    ...
    <x-modal name="rincian-belanja-modal-hapus" :show="$confirmingDeleteId !== null" maxWidth="md">
        <div class="p-6">
    ...
    {{-- Modal Konfirmasi Hapus Terpilih tab "jenis" (hapus massal) -
         permintaan user 2026-09-26, pola sama seperti Penerimaan Honor
         PTK. Modal Hapus Terpilih tab "BMD" ada TERPISAH di
         _tabel-bmd.blade.php (lihat catatan di sana). --}}
    <x-modal name="rincian-belanja-modal-hapus-terpilih" :show="$confirmingHapusTerpilih" maxWidth="md">
        <div class="p-6">
    ...
    {{-- Modal Tambah/Edit/Hapus tab "BMD" (permintaan user 2026-09-22) --}}
    @include('livewire.pendataan-bosp.rincian-belanja-modal._form-bmd')
</div>
