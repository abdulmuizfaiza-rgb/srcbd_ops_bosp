<x-slot name="header">
    <h2 class="font-semibold text-xl text-slate-800 leading-tight">
        {{ __('Rincian Belanja Barang Habis Pakai') }}
    </h2>
</x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                {{-- SATU LAPIS tab saja (4 tab Triwulan langsung) - TIDAK ADA tab
                     utama tambahan (beda dengan Rincian Belanja Modal), karena
                     menu ini hanya SATU jenis (permintaan user 2026-09-11, Part
                     21 poin 3). --}}
                <div class="border-b border-slate-200 px-4 sm:px-8 pt-4 pb-4">
                    <nav class="flex flex-wrap gap-3">
                        @foreach ($triwulanOptions as $value => $label)
                            ...
                            >
                                Rincian Belanja Barang Habis Pakai TW-{{ $value }}
                            </button>
                        @endforeach
                    </nav>
                </div>
