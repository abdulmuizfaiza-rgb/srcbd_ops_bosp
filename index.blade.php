...
<div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow sm:rounded-lg overflow-hidden">
                {{-- Tab Triwulan - user tidak menentukan warna khusus untuk menu
                     ini (beda dengan Penerimaan Honor PTK yang eksplisit minta
                     Biru/Ungu/Kuning/Hijau), jadi dipakai skema warna BAWAAN
                     komponen <x-tab-triwulan> (Sky/Emerald/Amber/Fuchsia) -
                     hanya teksnya ditulis custom "Daya & Jasa TW-n" di sini
                     (bukan lewat <x-tab-triwulan> langsung) supaya komponen
                     bersama itu tidak perlu diubah & menu lain (Lampiran
                     2a/2b/2c) tidak ikut terpengaruh. --}}
                ...
                <div class="p-4 sm:p-8">
                    <div class="flex flex-col xl:flex-row xl:flex-wrap xl:items-start xl:justify-between gap-3 mb-6">
                        <div class="flex flex-wrap sm:flex-row sm:items-center gap-2.5">
                            ...
                            <input wire:model.live.debounce.300ms="search" type="text" placeholder="Cari uraian pembayaran / sekolah..." class="border-slate-300 focus:border-blue-500 focus:ring-blue-500 rounded-md shadow-sm text-xs">
                        ...
                        <div class="flex flex-wrap sm:flex-row sm:items-center gap-2.5">
                            ...
                            <div class="flex items-center gap-1.5">
                                ...
                                <x-primary-button type="button" wire:click="tambah" class="whitespace-nowrap !px-2.5 !py-1.5 !text-[10px]">
                        ...
                        <table class="min-w-full divide-y divide-slate-200 text-xs">
                            <thead class="sticky top-0 z-10 bg-slate-50">
                                <tr class="text-left text-slate-500">
                                    ...
                                    <th class="px-2 py-2.5">Uraian Pembayaran</th>
                            ...
                            {{-- Sengaja pakai BEBERAPA <tbody> (bukan satu <tbody> berisi semua
                                 ...
                                 x-data sendiri (terbuka/tertutup) - pola SAMA PERSIS diterapkan
                                 di menu Penerimaan Honor PTK (2026-09-10, Part 16), sekarang
                                 diterapkan juga di sini (permintaan user 2026-09-10, Part 17
                                 poin 1). Simbol + di Kolom No (sebelum nomor) untuk
                                 membuka/menutup baris data yang sudah diinput tiap sekolah. --}}
                            @php $nomorSekolah = 0; @endphp
                            @forelse ($daftarSekolah as $sekolah)
                                @php
                                    $nomorSekolah++;
                                    $jumlahBaris = $sekolah->langgananDayaJasa->count();
                                    $nomorBaris = 0;
                                @endphp
                                <tbody wire:key="daya-jasa-grup-{{ $sekolah->id }}" x-data="{ terbuka: false }" class="divide-y divide-slate-100 border-b-2 border-slate-200">
                                    ...
                                    {{-- Baris placeholder kosong siap-isi, SELALU ada di akhir tiap
                                         sekolah - pola sama seperti Penerimaan Honor PTK (permintaan
                                         user 2026-09-10: "bentuk nya harus berupa tabel yang siap di
                                         input baik melalui tabel atau melalui form input nya"). Kunci
                                         ...
                                         lagi, siap dipakai untuk baris berikutnya di sekolah yang sama
                                         (Uraian Pembayaran BOLEH duplikat, sesuai jawaban
                                         AskUserQuestion "Boleh duplikat"). Tombol "+" pada kolom Aksi
                                         dibuat FUNGSIONAL sejak awal (fokus ke kotak pertama baris,
                                         murni Alpine sisi klien) - pelajaran dari koreksi tombol "+"
                                    ...
                                    {{-- Baris Jumlah Langganan Daya Jasa PER SEKOLAH - permintaan
                                         user 2026-09-16: "tambahkan baris Jumlah Langganan Daya
                                         Jasa (kolom 1 sampai kolom 7 di merge cell) dan kolom
                                         Jumlah di total kan". Sengaja ikut x-show="terbuka" +
                                         x-cloak (sama seperti baris detail lain) - jawaban
                                         AskUserQuestion "Per sekolah + Total seluruh sekolah":
                                         baris ini HANYA tampil saat grup sekolah dibuka, pola
                                         sama seperti Total Jumlah Honor Yang Diterima pada
                                         Penerimaan Honor PTK. Kolom 1-7 (No s.d. Tarif Harga)
                                         digabung jadi satu sel label (colspan="7"), nilai total
                            ...
                            {{-- Baris Jumlah Langganan Daya Jasa UNTUK SELURUH SEKOLAH -
                                 jawaban AskUserQuestion "Per sekolah + Total seluruh sekolah".
                                 BEDA dengan baris Jumlah per sekolah di atas (yang ikut
                                 tersembunyi/tampil bersama grupnya) - baris ini SELALU
                                 terlihat di paling bawah tabel, tidak collapsible, persis pola
                                 Total Jumlah Honor Yang Diterima Seluruh Sekolah pada
                                 Penerimaan Honor PTK. Nilainya ($totalDayaJasaKeseluruhan,
                                 dihitung di render()) otomatis ikut scope peran & filter yang
                                 sedang aktif karena dijumlahkan dari $daftarSekolah yang sama
                                 dengan yang dirender di atas. Kolom 1-7 digabung (colspan="7")
                                 sebagai label, nilai di kolom ke-8 (Jumlah). --}}
    ...
    <x-modal name="langganan-daya-jasa-form" :show="$showForm" maxWidth="lg">
        <form wire:submit="simpan" class="p-6">
            <h2 class="text-lg font-medium text-slate-900 mb-1">
                {{ $editingId ? 'Edit Data Langganan Daya dan Jasa' : 'Tambah Data Langganan Daya dan Jasa' }}
            </h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    <x-input-label for="uraian_pembayaran" value="Uraian Pembayaran" />
                    <x-text-input wire:model="uraian_pembayaran" id="uraian_pembayaran" class="block mt-1 w-full" type="text" placeholder="mis. Listrik, Internet, Air PDAM" />
                    <x-input-error :messages="$errors->get('uraian_pembayaran')" class="mt-2" />
                </div>
                ...
                <div>
                    ...
                    <x-text-input wire:model="satuan" id="satuan" class="block mt-1 w-full" type="text" placeholder="mis. Bulan" />
                ...
                >
                    ...
                    {{-- Dihitung LANGSUNG di sisi klien (Alpine) - pola sama seperti
                         perbaikan di menu Penerimaan Honor PTK (2026-09-10), supaya
                         kotak ini langsung berubah sambil mengetik di kotak
                         Volume/Tarif Harga, tanpa perlu form disimpan dulu. --}}
    ...
    <x-modal name="langganan-daya-jasa-hapus" :show="$confirmingDeleteId !== null" maxWidth="md">
        <div class="p-6">
