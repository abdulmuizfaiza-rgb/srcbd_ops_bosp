 ...
 * 2026-09-10, Part 17 poin 2).
 *
 * Punya DUA lapis tab: tab UTAMA ($tabUtama = 'barang'/'jasa', lihat
 * RincianPemeliharaan::JENIS_OPTIONS - "Rincian Pemeliharaan Bangunan"
 * vs "Rincian Jasa Pemeliharaan") DI ATAS tab Triwulan (1-4, pola sama
 * seperti Langganan Daya Jasa/Penerimaan Honor PTK) - belum pernah ada
 * menu dengan 2 lapis tab sebelum ini.
 *
 ...
 * mengikuti pola Langganan Daya Jasa PERSIS, hanya field-nya lebih
 * banyak (9 field: Kode UPB, Nama Barang, Nama Merk Barang, Volume,
 * Satuan, Harga Satuan, Asal Usul, Tanggal, Keterangan) - sesuai
 * jawaban AskUserQuestion 2026-09-10 ("Banyak baris per sekolah").
 *
 * Kolom "Nama Barang" SENGAJA TIDAK diberi validasi keunikan (boleh
 * duplikat, sesuai jawaban AskUserQuestion) - sama seperti Uraian
 * Pembayaran pada Langganan Daya Jasa.
 *
 ...
 * Harga Satuan - lihat RincianPemeliharaan::hitungTotalHarga()),
 ...
 */
#[Layout('layouts.app')]
#[Title('Belanja Pemeliharaan Bangunan')]
class Index extends Component
{
    ...
    #[Url(as: 'tab')]
    public string $tabUtama = RincianPemeliharaan::JENIS_BARANG;
    ...
    /**
     * Pindah tab UTAMA (Rincian Pemeliharaan Bangunan / Rincian Jasa
     * Pemeliharaan). Triwulan aktif TIDAK direset - kalau user sedang di
     * TW-2 lalu pindah tab utama, tetap di TW-2 pada tab utama yang baru.
     */
    public function pindahTabUtama(string $jenis): void
    {
        if (! array_key_exists($jenis, RincianPemeliharaan::JENIS_OPTIONS)) {
            return;
        }
    ...
    public function updated(string $name, mixed $value): void
    {
        ...
        $baris = RincianPemeliharaan::find($rowId);
        ...
        if (in_array($field, ['volume', 'harga_satuan'], true)) {
            ...
            $data['total_harga'] = RincianPemeliharaan::hitungTotalHarga($volumeBaru, $hargaBaru);
        }
     ...
     * berikutnya - pola identik updatedBarisBaru() Langganan Daya Jasa,
     * TIDAK ADA pengecekan duplikat apapun pada Nama Barang (sesuai
     * jawaban AskUserQuestion "boleh duplikat").
     */
    protected function updatedBarisBaru(string $name, int $idPlaceholder, string $field, mixed $value): void
    {
        ...
        $data['total_harga'] = RincianPemeliharaan::hitungTotalHarga(
        ...
        RincianPemeliharaan::create($data);
    ...
    public function tambah(): void
    {
        ...
        $this->showForm = true;
        $this->dispatch('open-modal', 'belanja-pemeliharaan-bangunan-form');
    }

    public function edit(int $id): void
    {
        $baris = RincianPemeliharaan::findOrFail($id);
    ...
    public function batal(): void
    {
        $this->showForm = false;
        $this->resetForm();
        $this->dispatch('close-modal', 'belanja-pemeliharaan-bangunan-form');
    }
    ...
    public function simpan(): void
    {
        ...
        // Langganan Daya Jasa/Penerimaan Honor PTK.
        $validated['tanggal'] = $validated['tanggal'] !== null && trim((string) $validated['tanggal']) !== ''
            ? $validated['tanggal']
            : null;

        $validated['total_harga'] = RincianPemeliharaan::hitungTotalHarga($validated['volume'], $validated['harga_satuan']);
        $validated['jenis'] = $this->tabUtama;
        $validated['tahun'] = $this->tahun;
        $validated['triwulan'] = $this->triwulan;

        if ($this->editingId) {
            $baris = RincianPemeliharaan::findOrFail($this->editingId);
            abort_unless($this->bolehKelola($baris->profil_sekolah_id), 403);
            $baris->update($validated);
        } else {
            $validated['created_by'] = auth()->id();
            RincianPemeliharaan::create($validated);
        }

        $this->showForm = false;
        $this->resetForm();
        $this->dispatch('close-modal', 'belanja-pemeliharaan-bangunan-form');
        session()->flash('status', 'Data Belanja Pemeliharaan Bangunan berhasil disimpan.');
    }
    ...
    public function konfirmasiHapus(int $id): void
    {
        ...
        $this->dispatch('open-modal', 'belanja-pemeliharaan-bangunan-hapus');
    }
    ...
    public function hapus(): void
    {
        ...
        $this->confirmingDeleteId = null;
        $this->dispatch('close-modal', 'belanja-pemeliharaan-bangunan-hapus');
        session()->flash('status', 'Data Belanja Pemeliharaan Bangunan berhasil dihapus.');
    }

    /**
     * Query flat (bukan per-sekolah) - dipakai HANYA oleh export(), sama
     * seperti pola queryDasar() pada Langganan Daya Jasa. Disaring juga
    ...
    protected function queryDasar()
    {
        $query = RincianPemeliharaan::query()
            ...
            ->where('rincian_pemeliharaan.triwulan', $this->triwulan)
        ...
        if (! $this->bolehKelolaSemua()) {
            ...
            $query->where('rincian_pemeliharaan.profil_sekolah_id', $this->filterSekolahId);
        }

        if ($this->search !== '') {
            $query->where(function ($q) {
                $q->where('rincian_pemeliharaan.nama_barang', 'like', "%{$this->search}%")
                    ->orWhere('rincian_pemeliharaan.kode_upb', 'like', "%{$this->search}%")
    ...
    public function import(): void
    {
        ...
        try {
            ...
            Excel::import(
                new RincianPemeliharaanImport($this->tabUtama, $this->tahun, $this->triwulan, auth()->id(), $sekolahDiperbolehkan),
                $this->fileImport->getRealPath()
            );

            $this->fileImport = null;
            session()->flash('status', 'Import Belanja Pemeliharaan Bangunan berhasil.');
    ...
    public function render()
    {
        ...
        if ($this->search !== '') {
            ...
            $query->where(function ($q) use ($cari) {
                $q->where('nama_sekolah', 'like', "%{$cari}%")
                    ...
                    ->orWhereHas('rincianPemeliharaan', function ($q2) use ($cari) {
        ...
        foreach ($daftarSekolah as $sekolah) {
            foreach ($sekolah->rincianPemeliharaan as $barisData) {
        ...
        // yang SEDANG DITAMPILKAN (permintaan user 2026-09-16: "tambahkan
        // baris Jumlah ... dan kolom Total Harga di total kan"; jawaban
        // AskUserQuestion: "Per sekolah + Total seluruh sekolah", sama
        // seperti Langganan Daya Jasa/Penerimaan Honor PTK) - dihitung dari
        ...
        // rincianPemeliharaan yang di-eager-load di atas sudah disaring per
        // jenis. Total PER SEKOLAH dihitung langsung di view lewat
        // $sekolah->rincianPemeliharaan->sum('total_harga') (koleksi sudah
        ...
        return view('livewire.pendataan-bosp.belanja-pemeliharaan-bangunan.index', [
